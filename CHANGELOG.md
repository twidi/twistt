...
- Hotkey could be re-activated while the previous session was still processing, causing overlapping sessions and mixed output. The hotkey is now ignored until the full pipeline has completed
- Missing spaces between words in post-treatment output with some LLM providers that stream full phrases as single chunks instead of sub-word tokens
